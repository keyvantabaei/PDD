...
detectorSD10cm::detectorSD10cm(const G4String& name,const G4String& HCname):
G4VSensitiveDetector(name),
_detectorSD10cmHC(NULL)
{
collectionName.insert(HCname);

}

detectorSD10cm::~detectorSD10cm(){}

void detectorSD10cm::Initialize(G4HCofThisEvent* hc)
{
_detectorSD10cmHC = new detectorSD10cmHC(SensitiveDetectorName,collectionName[0]);
G4int hcID = G4SDManager::GetSDMpointer()->GetCollectionID(collectionName[0]);
hc->AddHitsCollection(hcID,_detectorSD10cmHC);
}
G4bool detectorSD10cm::ProcessHits(G4Step* step,G4TouchableHistory*)
{
  //scattered and all photons

    if(step->GetTrack()->GetDefinition() == G4Gamma::Gamma())
    {
     auto hc = new detectorSD10cmHit();
...
void detectorSD10cm::EndOfEvent(G4HCofThisEvent*)
{
}