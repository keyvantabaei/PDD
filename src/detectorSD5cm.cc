...
detectorSD5cm::detectorSD5cm(const G4String& name,const G4String& HCname):
G4VSensitiveDetector(name),
_detectorSD5cmHC(NULL)
{
collectionName.insert(HCname);

}

detectorSD5cm::~detectorSD5cm(){}

void detectorSD5cm::Initialize(G4HCofThisEvent* hc)
{
_detectorSD5cmHC = new detectorSD5cmHC(SensitiveDetectorName,collectionName[0]);
G4int hcID = G4SDManager::GetSDMpointer()->GetCollectionID(collectionName[0]);
hc->AddHitsCollection(hcID,_detectorSD5cmHC);
}
G4bool detectorSD5cm::ProcessHits(G4Step* step,G4TouchableHistory*)
{
  //scattered and all photons

    if(step->GetTrack()->GetDefinition() == G4Gamma::Gamma())
    {
     auto hc = new detectorSD5cmHit();
...
void detectorSD5cm::EndOfEvent(G4HCofThisEvent*)
{
}